<template>
  <div class='content'>
    <div class='left'>
      ...
      <div class='ab'>
        <div style="width: 270px;height: auto" v-for="(design,key) in design_list" :key="key">
          <img :src="'http://192.168.1.233/yichang/' + key" alt="">
          <span class="tag-name" v-for="item in design.design_data">
            <span v-if="codeData[item.code]">{{ item.code + "" + codeData[item.code].category }}</span>
            <span v-if="codeData[item.code]">{{ "库存" + codeData[item.code].inventory }}</span>
          </span>

          <div v-for="(tags_list,tagsListKey) in design.design_data">
            <span v-if="codeData[tags_list.code]"
                  @click="showTagInfo=tags_list;tagContent('版型', tags_list.tags[0].id,tags_list.tags[0].notes);design_code = tags_list.code">{{
                tags_list.code + codeData[tags_list.code].category
              }}</span>
            <span v-if="showTagInfo===tags_list" class="tags" v-for="(item, tagKey) in tags_list.tags" :key="tagKey">
              <span class="tag-text" @click="delTag(item.id, tagKey);tags_list.tags.splice(tagKey,1)">-</span>
              <span class="del-tag" @click='design_code = tags_list.code;tagContent(item.tags, item.id,item.notes)'>{{
                  item.tags
                }}</span>
            ...
            <span class="add-tag" @click="addTagInput = tags_list.code" v-if="showTagInfo===tags_list">
              <el-input v-if="addTagInput === tags_list.code" style="width: 100px" v-model="addTagName"
                        @blur="addTags(addTagName, tags_list.id,key,tagsListKey)">
              </el-input>
              <span v-else>+</span>
            </span>
          </div>
        ...
        <div class='ab' :style="{ display: mixChangeStyle }" ref="myDiv">
          <div v-for="(item,key) in pairingList" :key="key">
            <span class="tag-name" v-for="code in item.child_code.split(',')">
              <span v-if="codeData[code]">{{ code + "" + codeData[code].category }}</span>
              <span v-if="codeData[code]">{{ "库存" + codeData[code].inventory }}</span>
            </span>
            <span class="tag-icon">
              <el-popconfirm
                  title="是否确认删除？"
...
const getDesign = () => {
  design.getDesign2(null).then(response => {
...
// const goodStyle = ref('')
const addTags = (name, design_id, key, tagsListKey) => {
  design.addTags(name, design_id).then(response => {
    design_list.value[key][tagsListKey]["tags"].push(name)
    addTagName.value = ''
    addTagInput.value = ''
    getDesign()
  }).catch(err => {
